...
/*----------------------------------------------------------------
Function name	: searchNodeDuplicate() - data와 일치하는 모든 노드를 검색 
Parameters		: lp - 리스트 관리 구조체의 주소
				  data - 검색할 데이터
				  count - 찾은 노드의 수를 저장하는 변수의 시작주소 
Returns			: 성공 - 찾은 모든 노드의 시작주소가 저장된 배열의 주소 
                  실패 - NULL pointer 
----------------------------------------------------------------*/
node_t **searchNodeDuplicate(list_t *lp, int data, int *count)
{
	node_t **p=NULL;  // realloc()함수 사용을 위해서 반드시 초기화  
	node_t *tp;

	*count = 0;  // count가 가리키는 변수 초기화 - searchNodeDuplicate()함수가 여러번 호출될 수 있으므로 반드시 필요 

	tp = lp->head->next;
	while(tp!=lp->tail){
		if(tp->data==data){
			++*count;
			p = realloc(p, sizeof(node_t *) * (*count)); // realloc()함수의 첫번째 전달인자가 NULL일 경우에는 malloc()함수처럼 동작함 
			p[*count-1] = tp;
		}
		tp=tp->next;
	}
